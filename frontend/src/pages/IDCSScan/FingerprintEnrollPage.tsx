...
async function captureFromScanner(
  ...
  /* ── ESP32 HTTP Bridge / Direct WiFi ─────────────────────── */
  if (type === 'esp32_bridge') {
    // If running in browser against ESP32 directly
    try {
      const statusRes = await fetch(`${url}/status`, { method: 'GET' }).catch(() => null);
      if (statusRes && statusRes.ok) {
        ...
        if (captureRes.ok) {
          ...
          if (captureData?.template_b64) {
            return {
              template_b64: String(captureData.template_b64),
              quality_score: Number(captureData.quality_score ?? 90),
              slot: captureData.slot ?? opts?.slot ?? 1,
      ...
      // Direct WiFi browser call had network issue, fallback to backend sync
    }

    // Fallback: Notify cloud backend that ESP32 should enroll into this target slot
    try {
      const syncRes = await fetch(`${getApiBase()}/api/idscan/biosecure/device/mode/`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          mode: 'ENROLL',
          target_slot: opts?.slot || 1,
          user_id: opts?.userId,
        }),
      });
      if (syncRes.ok) {
        return {
          template_b64: 'ESP32_HARDWARE_SLOT_REGISTERED',
          quality_score: 95,
          slot: opts?.slot || 1,
          user_id: opts?.userId,
        };
      ...
      console.warn('Backend mode sync fallback error:', backendErr);
    }

    throw new Error('Please place student finger on ESP32 sensor now.');
  }
...
export default function FingerprintEnrollPage() {
  ...
  /* ── Auto-disconnect & revert to Attendance Mode on Unmount or Page Exit ── */
  useEffect(() => {
    const handleBeforeUnload = () => {
      // Revert backend device mode back to ATTENDANCE on tab close / reload
      try {
        const payload = JSON.stringify({ mode: 'ATTENDANCE' });
        const url = `${getApiBase()}/api/idscan/biosecure/device/mode/`;
    ...
    return () => {
      ...
      // Revert device mode on unmount
      try {
        fetch(`${getApiBase()}/api/idscan/biosecure/device/mode/`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  const startAutoEnrollment = useCallback(async () => {
    ...
    for (let i = 0; i < FINGERS.length; i++) {
      ...
      try {
        ...
        console.warn('Slot allocation fallback:', slotErr);
      }

      // Signal ESP32 over Cloud Mode API
      try {
        await fetch(`${apiBase}/api/idscan/biosecure/device/mode/`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token()}`,
          ...
          body: JSON.stringify({
            ...
            reg_no: userInfo.identifier,
            finger: f.label,
          }),
        });
      } catch (e) {
        console.warn('Mode notification error:', e);
      }

      // Wait / poll for ESP32 to complete enrollment of this slot (up to 15s)
      for (let poll = 0; poll < 15; poll++) {
        ...
        try {
          // Check if slot was recorded in database by ESP32 or check mode revert
          const chkRes = await fetch(`${apiBase}/api/idscan/fingerprint/status/?user_id=${userInfo.user_id}`, {
            headers: { Authorization: `Bearer ${token()}` },
          });
          if (chkRes.ok) {
            const chkData = await chkRes.json();
            const enrolledList: string[] = chkData?.fingers || [];
            if (enrolledList.includes(f.key) || (chkData?.count && chkData.count > i)) {
              captureSuccess = true;
              break;
            }
          }

          // Check if device captured this target slot
          const modeRes = await fetch(`${apiBase}/api/idscan/biosecure/device/mode/`);
          if (modeRes.ok) {
            const mData = await modeRes.json();
            if (mData.last_captured_slot === targetSlot) {
              captureSuccess = true;
              break;
            }
          }
      ...
      if (captureSuccess) {
          ...
          prev.map((s) =>
            s.finger === f.key
              ? {
                  ...s,
                  status: 'enrolled',
                  template_b64: `SLOT_${targetSlot}_STORED`,
                  quality_score: 95,
                  slot: targetSlot,
                  errorMsg: null,
                }
        ...
        setSlots((prev) =>
          prev.map((s) =>
            s.finger === f.key ? { ...s, status: 'error', errorMsg: 'Sensor timeout / no finger' } : s,
          ),
        );
      }
    ...
    // Switch device mode back to attendance
    try {
      await fetch(`${apiBase}/api/idscan/biosecure/device/mode/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (!autoCancelRef.current) {
      setMessage({
        type: 'info',
        text: `Completed registration: ${enrolledCountAcc}/${FINGERS.length} fingers recorded on sensor.`,
      });
    }
      ...
      <div className="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden mb-6">
        <div className="px-6 py-4 bg-gray-50/70 border-b border-gray-100 flex items-center justify-between flex-wrap gap-2">
          ...
          {(usbPort || scannerOnline) && (
            <div className="flex items-center gap-2 flex-wrap">
              <div className="flex items-center gap-2 bg-amber-50 border border-amber-300 rounded-xl px-3.5 py-1.5 shadow-xs">
                <span className="w-2.5 h-2.5 rounded-full bg-amber-500 animate-ping flex-shrink-0" />
                <span className="text-xs font-bold text-amber-900">{usbDeviceName || 'WiFi ESP32 Scanner (Enrollment Mode Active)'}</span>
              </div>
              <button
                type="button"
                onClick={async () => {
                  try {
                    setDeviceConnecting(true);
                    if (scannerOnline && scannerDetectedType === 'esp32_bridge') {
                      await fetch(`${apiBase}/api/idscan/biosecure/device/mode/`, {
                        method: 'POST',
                        headers: {
                          'Content-Type': 'application/json',
                    ...
                    setMessage({
                      type: 'info',
                      text: '✓ Connection ended. ESP32 scanner shifted back to Live Attendance Scanning Mode.',
                    });
                  } catch (e: any) {
                    setUsbError('Error disconnecting scanner: ' + (e.message || String(e)));
                  } finally {
                    setDeviceConnecting(false);
                  }
                }}
                disabled={deviceConnecting}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 rounded-xl text-xs font-semibold shadow-xs transition cursor-pointer"
              >
                End Connection (Turn into Attendance Mode)
              </button>
            </div>
          )}
        </div>

        {/* Warning Banner when Enrollment Mode is Live */}
        {(usbPort || scannerOnline) && (
          <div className="mx-6 mt-4 p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-between gap-3 text-xs text-amber-900">
            <div className="flex items-center gap-2.5">
              <span className="w-2.5 h-2.5 rounded-full bg-amber-500 animate-pulse shrink-0" />
              <span>
                <strong>Attendance Scanning Paused:</strong> Scanner is locked in Enrollment Mode. Live student attendance records will <strong>NOT</strong> be accepted until this connection is ended or this window is closed.
              </span>
            </div>
          </div>
        )}
              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    setUsbDeviceName('WiFi ESP32 Scanner (Enrollment Mode Active)');
                    setMessage({
                      type: 'success',
                      text: '✓ WiFi ESP32 Connected in Enrollment Mode! Place finger on sensor to register.',
                    });
                  } catch (e: any) {
                    setUsbError('Failed to connect ESP32: ' + (e.message || String(e)));
                  } finally {
                    setDeviceConnecting(false);
                  }
                }}
                disabled={deviceConnecting}
                className="inline-flex items-center gap-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 rounded-2xl px-4 py-2.5 text-xs font-semibold border border-indigo-200 transition shadow-xs"
              >
                Connect WiFi ESP32
              </button>
            </div>
