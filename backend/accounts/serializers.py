...
def _compute_effective_role_names(user) -> list[str]:
    ...
    # Isolated admin mode: for SUPER_ADMIN / Django superuser, only use
    # explicit accounts.Role memberships and do not derive roles from
    # staff-linked tables (DepartmentRole / RoleAssignment).
    try:
        is_admin_isolated = bool(getattr(user, 'is_superuser', False))
    except Exception:
        is_admin_isolated = False
    ...
    if 'SUPER_ADMIN' in existing:
        is_admin_isolated = True

    if is_admin_isolated:
